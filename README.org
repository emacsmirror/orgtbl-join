...
* New
Blocks surrounded by =#+begin:= =#+end:= may contain an old table, and
also several tags like =#+name:=, =#+plot:=, =#+tblfm:= and any other
information.

Upon refresh (=C-c C-c=) or wizard update (=C-c C-x x=) the content of the
block is preserved.

Previously, only =#+tblfm:= was preserved (and enriched by additional
formulas in a possible =:formula= parameter).

Also, the directory part is simplified when querying a file name.
...
in the column serving the join process. Use the =C-u= prefix to invoke
the wizard in expert mode.

The menu entry =Table > Column > Join with tables= is equivalent to =M-x
orgtbl-join=, if this menu entry has been added (see [[#installation][Installation]] for
how to do that).
...
- or =C-c C-x x =join=.
- prefix any of the above two commands with =C-u= to use expert mode.
...
The basic mode (as opposed to expert mode) only queries for input
tables in the current buffer. Of course, if the wizard updates an
existing block with exotic parameters, those parameters will be
queried even in basic mode.

As there might be as many reference tables as wanted, the wizard
continues asking for reference tables (in expert mode). When done,
answer =n= when the wizard ask if you want an additional reference table
to be joined.

The wizard does not (yet) take into account the =:cond= and =:post=
...
Suppose we have a join block like this one:

#+begin_example
#+BEGIN: join :mas-table "my_table" :mas-column "the_column" :ref-table "other_table" :ref-column "another_column"
#+END:
#+end_example
...
#+BEGIN: join
#+join: :mas-file
#+join: :mas-name my_table
#+join: :mas-orgid
#+join: :mas-params
#+join: :mas-slice
#+join: :mas-column the_column
#+join: :ref-file
#+join: :ref-name other_table
#+join: :ref-orgid
#+join: :ref-params
#+join: :ref-slice
#+join: :ref-column another_column
#+join: :full
#+join: :cols
#+join: :cond
#+join: :post
...
All fields recognized by OrgtblJoin are displayed, even when they
...
- Wizard can now be called in basic or expert mode.
- Preserve every side lines in =#+begin:= … =#+end:= blocks upon refresh
  or wizard update.
- Simplify directory part when querying a file name.
- Centralize cell→string in =-cell-to-string= which is the fastest.
